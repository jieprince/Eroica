...
public class RpcUtils {
   ...
   // private static final AtomicLong INVOKE_ID = new AtomicLong(0);
     ...
     * @param inv
     */
    /*public static void attachInvocationIdIfAsync(URL url, Invocation inv,RpcContext context){
    	if (isAttachInvocationId(url, inv,context) && getInvocationId(inv) == null && inv instanceof RpcInvocation) {
    		((RpcInvocation)inv).setAttachment(Constants.ID_KEY, String.valueOf(INVOKE_ID.getAndIncrement()));
        }
    }
    
    private static boolean isAttachInvocationId(URL url , Invocation invocation,RpcContext context) {
    	String value = url.getMethodParameter(invocation.getMethodName(), Constants.AUTO_ATTACH_INVOCATIONID_KEY);
    	if ( value == null ) {
    		//异步操作默认添加invocationid
    		return isAsync(url,invocation,context) ;
    ...
    public static boolean isAsync(URL url, Invocation inv) {
    	boolean isAsync ;
    	//如果Java代码中设置优先.
    	if (Boolean.TRUE.toString().equals(RpcContext.getContext().get(Constants.ASYNC_KEY))) {
    		isAsync = true;
    	} else {
