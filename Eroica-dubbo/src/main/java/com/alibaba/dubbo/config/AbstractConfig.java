...
public abstract class AbstractConfig implements Serializable {
    ...
    public void appendAnnotation(Class<?> annotationClass, Object annotation) {
    ...
    public  void appendProperties(Properties properties) {
    	appendSpecifyProperties(this,properties);
    }
    
    protected static void appendSpecifyProperties(AbstractConfig config,Properties properties) {
    	 if (config == null || properties==null || properties.size()==0) {
             return;
         }
         ...
         for (Method method : methods) {
             try {
                 ...
                 if (name.length() > 3 && name.startsWith("set") && Modifier.isPublic(method.getModifiers()) 
                     ...
                     if (value != null && (strValue=value.toString()).length()>0) {
                         method.invoke(config, new Object[] {convertPrimitive(method.getParameterTypes()[0], strValue)});
                     }
                 }
             } catch (Exception e) {
                 logger.error(e.getMessage(), e);
