...
public class DefaultSARDispatcherBean implements SARDispatcher,InitializingBean,ApplicationContextAware{
	...
	public  ServiceResponse handleRequest(ServiceRequest request){
		try{
			...
			if(esaFilters!=null && esaFilters.size()>0){
				...
				if(dispatcher==null){
					throw new IllegalArgumentException("Not found dispatcher for action<"+request.getRequestedServiceID()+"> .");
				}
	...
	public void afterPropertiesSet() throws Exception {
		...
		Map<String, ESAExceptionResolver> matchingBeans =this.getApplicationContext().getBeansOfType(ESAExceptionResolver.class); 
	...
	public synchronized Collection<ESADefinition> getESADefinitions() {
		...
		if(esaDefinitions==null || esaDefinitions.size()==0)return null;
